...
function NewSessionForm({ onCreate, busy }: { onCreate: (topic: string, goal: string) => void; busy: boolean }) {
...
function App() {
  ...
  const clearSessionAndBeginNew = async () => {
    ...
    try {
      ...
      discardedSessionIds.current.delete(session.id);
      setError((cause as Error).message);
    }
  };
    ...
    <div className="app-shell">
      ...
      {!session ? (
        <main className="empty-stage">
          ...
          <div className="empty-copy"><div className="eyebrow">New inquiry</div><h1>Put a difficult idea at the center of the table.</h1><p>Momo and Bobby debate the substance. Sam questions, judges, and guides where the learning goes next.</p><NewSessionForm onCreate={createSession} busy={busy} /></div>
        </main>
      ) : concluded ? (
            ...
            {confirmNewSession && (
              <div className="new-session-confirm" role="dialog" aria-labelledby="new-session-confirm-title" aria-modal="true">
                <div>
                  ...
                  <p>The transcript, summaries, evaluation, and uploaded sources will be permanently cleared when the new table starts. None of these steps is required.</p>
                </div>
                <div className="new-session-confirm-actions">
                  ...
                  <button className="button button-stop" onClick={clearSessionAndBeginNew}>No, start new roundtable</button>
                </div>
              </div>
            )}
