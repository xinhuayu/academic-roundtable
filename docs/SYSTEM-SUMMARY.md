...
Reasoning, output allowances, and timeouts are task-aware. Fast live turns default to low reasoning and target 60–110 visible words. Momo uses an 800-token base live allowance. Bobby retains a 1,400-token visible-response basis, but Gemini completion ceilings reserve hidden-thinking room: 4,096 Fast, 12,288 Research, and 32,768 Verification, with an absolute 65,536 cap. These ceilings do not increase the requested prose length. Bobby's Fast route is Gemini 3.5 Flash-Lite. Research mode defaults to GPT-5.6 Sol and Gemini 3.6 Flash with medium reasoning, 2.75× live token and 2.5× live timeout multipliers, a focused 140–220-word visible target per AI, and 1.5× background digest deadlines; Gemini adds another 1.35× deadline margin. Verification mode defaults to GPT-5.6 Sol and Gemini 2.5 Pro with high reasoning, 2× live token and 2.5× live timeout multipliers, 2× background digest deadlines, and another 1.5× Gemini deadline margin. An explicit Sam request to check the original source automatically selects Verification for that segment, but raw excerpts remain gated by that request. A voice-derived or otherwise long Sam contribution is retained throughout every AI round in its segment and receives the configured long-input multiplier across output capacity plus first-token, stream-idle, and total-turn deadlines. A Chat Completions `finish_reason` of `length` is persisted as interrupted, reported to Sam, and stops the segment before the next AI speaks. Connection, first-token, stream-read, and total-turn deadlines are independently configurable per participant. Background section and whole-job deadlines remain configurable through environment settings. Sam's interrupt still cancels the active stream task immediately, retaining any partial response already received.
...
- All 6 frontend tests and the production build pass. Frontend coverage includes actual per-turn model-route display, English and translated provenance formatting, ephemeral digest cards, landing-page multi-source staging, and the voice privacy/review states.
...
- Updated Bobby's stable workload routing: Gemini 3.5 Flash-Lite/low for Fast, Gemini 3.6 Flash/medium for Research, and Gemini 2.5 Pro/high for Verification. Model selection remains configurable through environment overrides.
