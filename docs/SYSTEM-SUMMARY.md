...
Language selection is persisted as `conversation_language` plus `language_source`. Explicit requests from Sam override the default or an automatically detected document language and cannot subsequently be replaced by another upload. Automatic detection uses conservative script and language-marker heuristics; mixed-language or low-quality OCR sources may require an explicit Sam instruction. The same constrained tag is appended to live dialogue and all source, topic, conversation, final, and one-page synthesis tasks. JSON keys stay stable while visible string values use the selected language.
...
Closeout is coordinated with the active generation lock so interrupted text is persisted before one frozen summary snapshot is assembled. The snapshot contains five explicit evidence classes: bounded extracted source text with filename/page/evidence labels, processed document digests, the Topic Digest, the complete periodic/requested Conversation Digest history, and the complete substantive Momo/Bobby/Sam transcript. Uploaded binary files, greetings, duplicate visible recap messages, closing pleasantries, and prior final artifacts are excluded. Extracted text is marked as untrusted evidence rather than instructions and is fairly clipped across passages if necessary to keep the request within context limits. Momo then generates the comprehensive Summary Digest while Bobby independently generates the one-page learning summary from that same snapshot; neither waits for or claims access to the other's output. Both jobs always select their Verification profiles with high reasoning regardless of the conversation's current mode: Momo uses the configured GPT-5.6 Sol verification route and Bobby uses the configured Gemini 2.5 Pro verification route, with verification-grade token and timeout allowances. The session remains `CLOSING` until both jobs settle, and each artifact has an independent digest-based fallback. Streaming cleanup cannot overwrite `CLOSING` or `CLOSED`. Sam may cancel both summary jobs together; the session then closes with its transcript and existing digests intact. At `CLOSED`, Sam may download the two synthesis artifacts independently, the readable transcript, or the complete archive. The archive includes explicit JSON files for the Topic Digest, latest Conversation Digest, complete digest history, and processed-source digests. Sam may save a learning evaluation included in every export. Downloading, reviewing, and evaluating are optional: when warned about unsaved data, Sam can select **No, start new roundtable** to purge the old session and its evaluation and proceed immediately.
...
- Persistent multilingual conversation state, source-language initialization, Sam-authoritative language switching, and per-task output-language enforcement
...
- Verification-grade closeout: Momo's comprehensive Summary Digest and Bobby's one-page summary always use their high-reasoning Verification routes, run concurrently from one frozen record, and remain independently downloadable
...
- All 10 frontend tests and the production build pass. Frontend coverage includes actual per-turn model-route display, English and translated provenance formatting, ephemeral digest and timeout-retry cards, parallel closeout author/status copy, landing-page multi-source staging, voice privacy/review states, localized turn-reminder copy, last-speaker voice selection, and the accessible reminder toggle.
...
- Current Bobby defaults have since moved to Gemini 3.5 Flash-Lite / 3.6 Flash / 2.5 Pro for Fast / Research / Verification and require a fresh live benchmark after restart.
...
- Updated Bobby's stable workload routing: Gemini 3.5 Flash-Lite/minimal for Fast, Gemini 3.6 Flash/medium for Research, and Gemini 2.5 Pro/high for Verification. Model selection remains configurable through environment overrides.
...
- Forced both closeout authors onto deep Verification profiles regardless of the live discussion mode: Momo uses GPT-5.6 Sol/high and Bobby uses Gemini 2.5 Pro/high with their larger budgets and latency margins. Closeout supplies bounded extracted text but never uploads the original source binaries to either provider.
