...
- Frontend: **8 Vitest tests passed**, followed by a successful TypeScript/Vite production build. Coverage includes actual model/mode/reasoning disclosure per AI turn, translated provenance-label formatting, the ephemeral timeout-retry System card, and parallel Momo/Bobby closeout status in addition to the existing voice, upload, and digest-status behavior. A local browser audit confirmed that Voice input, its privacy notice, the four composer controls, textarea, and Answer button fit within Sam's fixed panel without scrolling or console errors. The voice control displays elapsed time without a duration ceiling and waits for Sam to stop.
