...
- [ ] Run **70 backend tests**, the frontend test suite, and the frontend production build from the clean copy.
...
- [x] 70 backend tests passed.
- [x] Frontend Vitest suite passed (4 tests, including ephemeral digest-status messages, landing-page multi-source staging, and Sam voice privacy/review states).
...
- [x] Rebuilt GitHub submission archive has 58 entries and contains no forbidden local/runtime files. Record the final SHA-256 from the packaging run before publication.
