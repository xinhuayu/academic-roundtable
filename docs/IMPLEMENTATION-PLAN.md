...
- Research routes Momo to GPT-5.6 Luna with `xhigh` reasoning and Bobby to Gemini 3.6 Flash with medium reasoning by default, with 2.75× live token allowances, 2.5× live deadlines, a focused 140–220-word target, and 1.5× background digest deadlines. Gemini adds a 12,288-token completion floor and 2× deadline margin. This keeps the frequent research path economical while reserving Sol for Verification.
...
- Background source, Topic, and Conversation Digest jobs use Research by default, even for Fast live sessions; only an explicitly Verification-profile session promotes those jobs to Verification. Closeout summaries continue to use the mode Sam selects.
