...
- Sam can choose Fast discussion, Research mode, or Verification mode. Research uses the economical deep-reasoning route (Luna `xhigh` for Momo and Gemini 3.6 Flash `medium` for Bobby); Verification reserves Sol `high` and Gemini `high` for deep methodological checking. Both deeper profiles have larger, longer budgets; Fast remains the low-latency default.
...
- On the landing page, changing the mode immediately updates the model labels shown under Momo and Bobby; those labels are the configured routes that will be used when the table starts.
...
- Temporary local System cards for active source-document processing, Topic Digest, and Conversation Digest work; these disappear on completion and are never stored or exported
...
# Longer limits apply to deeper-reasoning background synthesis.
...
- **Research mode:** GPT-5.6 Luna with `xhigh` reasoning for Momo and Gemini 3.6 Flash with medium reasoning for Bobby by default, 2.75× live token allowances and 2.5× live deadlines. Bobby additionally receives the 12,288-token Gemini completion floor and 2× Gemini latency margin. Each AI is asked for a focused 140–220-word contribution in two connected paragraphs, including the relevant inferential, methodological, statistical, mathematical, or theoretical detail.
...
Gemini thinking is not free or fully visible: thought tokens count toward billed output. The protected ceilings prevent premature truncation, while `reasoning_effort` remains the actual cost/latency control. Keep Fast on minimal, Research on Luna `xhigh` / Gemini medium, and use Sol high Verification selectively.
